...
class EntityRenderer
  ...
  def prepare_texture_model(model : TextureModel)
    ...
    # texture atlas, load number_of_rows
    @shader.load_number_of_rows(model.number_of_rows)
  ...
  def prepare_instance(entity : Entity)
    ...
    #
    # load the texture x and y offset for each model
    # as this will or can differ for each model
    #
    @shader.load_offset(entity.get_texture_x_offset(),entity.get_texture_y_offset())
