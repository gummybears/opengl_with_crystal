...
class Game
  ...
  #
  def setup_lights()
    @lights << Light.new(@config.light_position(1),@config.light_color(1))
    @lights << Light.new(@config.light_position(2),@config.light_color(2))
    @lights << Light.new(@config.light_position(3),@config.light_color(3))
    @lights << Light.new(@config.light_position(4),@config.light_color(4))
  end
  ...
  def setup_terrains()
    ...
    # create our terrains
    @terrains << TerrainHeightMap.new(-1f32,-2f32, texture_pack, blend_map, "res/heightmap.png")
    ...
    @terrains << TerrainHeightMap.new( 0f32,-2f32, texture_pack, blend_map, "res/heightmap.png")
  ...
  def setup_models()
    ...
    #
    @models["fern"] = load_model_with_texture_atlas("fern", false,false, "res/fern_texture_atlas.png", 2)
  end
  ...
  def position_model_in_world(name : String)
