...
class Display
  ...
  def initialize(title : String, width : Float32, height : Float32, fov : Float32, near : Float32, far : Float32, bg : Color)
    ...
    # create a new window
    #
    @window = Window.new(width: width, height: height,title: title, hints: hints) #, monitor: CrystGLFW::Monitor.primary)
  ...
  # render a model
  #
  def render(entities : Array(Entity), shaderprogram : ShaderProgram, camera : Camera)
