...
float get_distance(in vec3 p){
  ...
  float r = 1.0;
  vec3 sphere_pos = vec3(0.0,1.0,6.0);

  float sphere_dist   = sd_sphere(p, sphere_pos, r);
  float plane_dist    = sd_plane(p);
  float capsule_dist  = sd_capsule(p, vec3(0,1,6), vec3(1,2,6), 0.2);
  float torus_dist    = sd_torus(p - vec3(0,0.5,6), vec2(1.5,0.3));
  float box_dist      = sd_box(p - vec3(-3,0.75,6), vec3(0.75));
  float cylinder_dist = sd_cylinder(p, vec3(0,0.3,3), vec3(3,0.3,5), 0.2);

  //float d = min(sphere_dist,plane_dist);
  float d = min(capsule_dist,plane_dist);
  // add the torus, box and cylinder
  d = min(d, torus_dist);
  d = min(d, box_dist);
