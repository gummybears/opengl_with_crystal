...
class Program
  ...
  def compile(vertex_source : Array(String), fragment_source : Array(String))
    ...
    vertex_text   = vertex_source.join("\n")
    fragment_text = fragment_source.join("\n")

    @vs_id = load_shader(vertex_text, LibGL::VERTEX_SHADER)
    @fs_id = load_shader(fragment_text, LibGL::FRAGMENT_SHADER)
  ...
  def compile(vertexfile : String, fragmentfile : String)
    ...
    pre1 = Preprocess.new()
    pre1.read(vertexfile)
    vertex_lines = pre1.lines

    pre2 = Preprocess.new()
    pre2.read(fragmentfile)
    fragment_lines = pre2.lines

    compile(vertex_lines,fragment_lines)
  end
